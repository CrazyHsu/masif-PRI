...
# def train_masif_site(learning_obj, params, batch_size=100, num_iterations=100, num_iter_test=1000,
...
#                 chains2 = ''
#             pids = []
#             if str(pdbid) + "_" + str(chains1) in training_list:
#                 pids.append("p1")
#             if str(pdbid) + "_" + str(chains2) in training_list:
#                 pids.append("p2")
#             for pid in pids:
#                 try:
...
#                 chains2 = ''
#             pids = []
#             if str(pdbid) + "_" + str(chains1) in testing_list:
#                 pids.append("p1")
#             if str(pdbid) + "_" + str(chains2) in testing_list:
#                 pids.append("p2")
#             for pid in pids:
...
def train_masifPNI_site(argv):
    ...
    for num_iter in range(num_iterations):
        ...
        for pdb_id in data_dirs:
            ...
            # pdbid = ppi_pair_id.split("_")[0]
            pChain = set(pni_pairs_df.loc[pni_pairs["PDB_id"] == pdb_id]["pChain"].to_list())
            if len(ppi_pair_id.split("_")) > 2:
                chains2 = ppi_pair_id.split("_")[2]
            else:
                chains2 = ''
...
def train_masifPNI_site1(argv):
    ...
    #     logfile.write("{}: {}\n".format(key, params[key]))
    #
    # BoundTuple = namedtuple("BoundTuple", ["PDB_id", "pChain", "naChain", "naType"], defaults=[""] * 4)
    # training_list = open(params["training_list"]).readlines()
    # training_list = [BoundTuple(*x.rstrip().split("_")) for x in training_list]
    # training_df = pd.DataFrame(training_list)
    # training_df_gp = training_df.groupby(["PDB_id", "pChain"])
    #
    # testing_list = open(params["testing_list"]).readlines()
    # testing_list = [BoundTuple(*x.rstrip().split("_")) for x in testing_list]
    ...
    #     for pdb_id in data_dirs:
    #         mydir = os.path.join(params["masif_precomputation_dir"], pdb_id)
    #         pChains = set(pni_pairs_df.loc[pni_pairs["PDB_id"] == pdb_id]["pChain"].to_list())
    #
